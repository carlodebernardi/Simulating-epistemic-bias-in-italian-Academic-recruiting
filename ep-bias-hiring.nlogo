;
;    ep-bias-hiring - ABM simulation of the italian academic recruitment
;    Copyright (C) 2019  Carlo Debernardi, Eleonora Priori, Marco Viola
;
...
globals [
  ...
  concorsi-aperti     ;; posti vacanti (dopo applicazione turnover)
...
to crea-ricercatori [scuola]
  ...
  ;; crea ricercatori (uno per prof), usata ad intervalli regolari definiti da incremento-ricercatori
...
to-report cv-years [n scuola]
  ...
  ;; per ricercatori e professori neonati, serve a calcolare il cv per ogni anno di carriera
...
to-report bias-pubblicazione [scuola]
  ...
  ;; calcolo il bias di pubblicazione, è un coefficiente dell'incremento annuo del cv che dipende linearmente dalla quota di agenti della scuola data sul totale, varia tra un minimo ed un massimo definiti da input
...
setup</setup>
  ...
  <experiment name="12 - #concorso nob" repetitions="1" runMetricsEveryStep="false">
    <setup>reset
setup</setup>
    <go>go</go>
    <enumeratedValueSet variable="until">
  ...
  <experiment name="13 - #asn nob" repetitions="1" runMetricsEveryStep="false">
    <setup>reset
setup</setup>
    <go>go</go>
    <enumeratedValueSet variable="until">
